<x-owner-layout title="Waralaba" active="waralaba" style="margin-top: 0;">
                        ...
                        <table class="waralaba-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: left;">{{ $waralaba->waralaba_name }}</td>
                                    <td style="text-align: left;">{{ $waralaba->concept }}</td>
                                    <td style="text-align: left;">Rp.
                                        {{ number_format(floatval($waralaba->price), 0, ',', '.') }}</td>
                                    <td class="text-center">{{ $waralaba->soldCount() }}</td>
                                    <td style="text-align: left;">
                                        {{ implode(' ', array_slice(str_word_count($waralaba->description, 1), 0, 8)) }}..
                                    </td>
                                    <td class="text-center">
                                        <div class="waralaba-details"
                                            style="display: flex; gap: 5px; align-items: center; justify-content: center;">
                                            <a href="{{ route('owner.waralaba.edit', $waralaba->id) }}"
                                                class="btn btn-circle btn-warning"
                                                style="background-color: #FFC107; border: none;"><i class="fas fa-edit"
                                                    style="color: white;"></i></a>
                                            <form action="{{ route('owner.waralaba.destroy', $waralaba->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
    ...
    document.getElementById("searchInput").addEventListener("input", function() {
        var input, filter, table, tr, tdName, tdId, tdConcept, tdPrice, i, txtValueName, txtValueId,
            txtValueConcept, txtValuePrice;
        input = this;
        ...
        for (i = 0; i < tr.length; i++) {
            ...
            if (tdName || tdId || tdConcept || tdPrice) {
                ...
                if (txtValueName.toUpperCase().indexOf(filter) > -1 || txtValueId.toUpperCase().indexOf(
                    filter) > -1 || txtValueConcept.toUpperCase().indexOf(filter) > -1 || txtValuePrice
